...
function suggestPostal(q) {
    ...
    if (/^\d{1,2}$/.test(num)) {
        const want = num;
        const matches = [];
        Object.keys(byPrefix).forEach(pref => { if (pref.startsWith(want))
            matches.push(...(byPrefix[pref] || [])); });
    ...
    if (!byCity[key]) {
        ...
        if (key.endsWith('ick')) {
            ...
            if (byCity[alt])
                return byCity[alt].slice(0, 20).map(e => ({ code: e.code, city: e.city, label: `${e.code} ${e.city}` }));
        }
    ...
    if (matches.length > 0)
        return matches.slice(0, 30).map(e => ({ code: e.code, city: e.city, label: `${e.code} ${e.city}` }));
...
function lookupPostalPrefixes(cityOrPostal) {
    ...
    if (byCity[key])
        return Array.from(new Set(byCity[key].map(e => e.code.slice(0, 3))));
