...
export function getPool(conn?: string) {
  if (pool) return pool;
  const connectionString = conn || process.env.PG_CONN || process.env.DATABASE_URL || '';
  if (!connectionString) throw new Error('Postgres connection string not provided in PG_CONN or DATABASE_URL');
  pool = new Pool({ connectionString });
  return pool;
}

export async function init(conn?: string) {
...
export default { init, upsertItem, existsById };
