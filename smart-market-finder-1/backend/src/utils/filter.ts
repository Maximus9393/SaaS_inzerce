...
export function filterResults(items: Item[] = [], opts: { method?: string; keywords?: string } = {}) {
...
function score(item: Item, kw: string) {
  if (!kw) return 0;
  let s = 0;
  const t = (item.title || '').toLowerCase();
  if (t.includes(kw)) s += 10;
  if ((item.title || '').length > 0) s += 1;
  return s;
}
